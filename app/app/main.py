...
from .schemas import Token, TokenData, User, UserInDB, UserCreate, HTTPError, UserUpdate
...
@app.put("/users/{id}", response_model= User, responses={HTTP_404_NOT_FOUND: {"model": HTTPError}}, tags=["users"],)
def update_user(*, db: SessionLocal = Depends(get_db), id: UUID4, user_in: UserUpdate,) -> Any:
...
@app.get("/users/{id}", response_model=User, responses={HTTP_404_NOT_FOUND: {"model": HTTPError}}, tags=["users"],)
def get_user(*, db: SessionLocal = Depends(get_db), id: UUID4) -> Any:
    user = actions.user.get(db=db, id=id)
    if not user:
        raise HTTPException(status_code=HTTP_404_NOT_FOUND, detail="User not found")
    return user
...
@app.delete("/users/{id}", response_model=User, responses={HTTP_404_NOT_FOUND: {"model": HTTPError}}, tags=["users"], )
def delete_user(*, db: SessionLocal = Depends(get_db), id: UUID4) -> Any:
    user = actions.user.get(db=db, id=id)
    if not user:
        raise HTTPException(status_code=HTTP_404_NOT_FOUND, detail="User not found")
